    ...
    match &*engine_guard {
        ...
        TranscriptionEngine::Sherpa(config) => {
            ...
            if let Some(joiner) = &config.joiner {
                // Transducer Mode
                args.push(format!("--encoder={}", config.encoder));
                ...
                args.push(format!("--whisper-decoder={}", config.decoder));
...
async fn cmd_load_model(model_path: String, state: State<'_, AppState>) -> Result<String, String> {
    ...
    if path.is_dir() {
        ...
        // Check file existence (Joiner is optional now)
        if !tokens.exists() || !encoder.exists() || !decoder.exists() {
            return Err(
                "Missing model files: tokens.txt, encoder.onnx/int8.onnx, decoder.onnx/int8.onnx"
                    .to_string(),
            );
        }
